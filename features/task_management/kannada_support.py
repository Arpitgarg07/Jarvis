...
class KannadaVoiceProcessor:
    ...
    def translate_priority_to_english(self, kannada_priority: str) -> str:
        ...
        reverse_translations = {v: k for k, v in self.translations.PRIORITY_TRANSLATIONS.items()}
        ...
        for pattern, pattern_type in kannada_time_patterns:
            ...
            if match:
                if pattern_type == 'today':
                    ...
                    now = datetime.now()
                    return f"{now.strftime('%Y-%m-%d')} {time_part}", command.replace(match.group(0), '').strip()
                
                elif pattern_type == 'tomorrow':
                    time_part = match.group(1)
                    tomorrow = datetime.now() + timedelta(days=1)
                    return f"{tomorrow.strftime('%Y-%m-%d')} {time_part}", command.replace(match.group(0), '').strip()
                
                elif pattern_type == 'day_name':
                    day_name = match.group(1)
                    # Convert Kannada day names to English for processing
                    day_mapping = {
                        'ಸೋಮವಾರ': 'monday', 'ಮಂಗಳವಾರ': 'tuesday', 'ಬುಧವಾರ': 'wednesday',
                        'ಗುರುವಾರ': 'thursday', 'ಶುಕ್ರವಾರ': 'friday', 'ಶನಿವಾರ': 'saturday', 'ಭಾನುವಾರ': 'sunday'
                    }
