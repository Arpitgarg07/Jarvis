...
class VoiceCommandParser:
    ...
    def _parse_english_command(self, command: str) -> Dict:
        ...
        # Set priority
        if re.match(
            r"set task priority:?\s*(.+?)\s+to\s+(high|normal|low|urgent)", command
        ):
            match = re.match(
                r"set task priority:?\s*(.+?)\s+to\s+(high|normal|low|urgent)", command
            )
    ...
    def _parse_kannada_command(self, command: str) -> Dict:
        ...
        if any(
            ...
            for pattern in self.kannada_processor.translations.KANNADA_COMMANDS[
                "add_task"
            ]
        ):
            title = self.kannada_processor.extract_kannada_task_title(
                command, "add_task"
            )
            if title:
                return self._parse_add_task(title)
        ...
        priority_patterns = self.kannada_processor.translations.KANNADA_COMMANDS[
            "set_priority"
        ]
        for pattern in priority_patterns:
            ...
            if match:
                ...
                english_priority = self.kannada_processor.translate_priority_to_english(
                    kannada_priority
                )
...
class TaskVoiceInterface:
    ...
    def process_voice_command(self, command: str) -> str:
        ...
        if action == "add_task":
            ...
            if language == Language.KANNADA:
                return self.kannada_processor.get_kannada_response(
                    "task_added", parsed["title"]
                )
            else:
            ...
            if tasks:
                ...
                if language == Language.KANNADA:
                    return self.kannada_processor.get_kannada_response(
                        "task_completed", parsed["task_title"]
                    )
                else:
                ...
                if language == Language.KANNADA:
                    return self.kannada_processor.get_kannada_response(
                        "task_not_found", parsed["task_title"]
                    )
                else:
            ...
            if tasks:
                ...
                if language == Language.KANNADA:
                    return self.kannada_processor.get_kannada_response(
                        "priority_updated", parsed["task_title"]
                    )
                else:
