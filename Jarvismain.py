    ...
    while True:
        ...
        if "wake up" in query or "start" in query or "makeup" in query or "breakup"in query:
            ...
            while True:
                ...
                elif "ai mode" in query or "ask ai" in query or "ask gpt" in query or "ask llm" in query:
                    from jarvis_ai import JarvisAI

                    ai_prompt = query
                    for trigger in ["ai mode", "ask ai", "ask gpt", "ask llm"]:
                        ai_prompt = ai_prompt.replace(trigger, "", 1)
                    ai_prompt = ai_prompt.replace("jarvis", "", 1).strip()

                    if not ai_prompt:
                        Speak("AI mode is ready. What should I do?")
                        ai_prompt = TakeCommand()

                    answer = JarvisAI().answer(ai_prompt)
                    print(f"Jarvis AI: {answer}")
                    Speak(answer)
                ...
                #^ Playing playlist from youtube.
                    ...
                    url = "https://youtube.com/@dinoCoder  "
                    Speak("Channel opening...")
                    webbrowser.get('chrome').open(url)
                    print("Channel opening...")

                elif 'channel dashboard' in query:
                    url = "https://studio.youtube.com/channel/UCAi-EONczHNaAqr_Ff3HRsA"
                    Speak("Channel dashboard opening...")
                    webbrowser.get('chrome').open(url)
                    print("Channel dashboard opening...")

                elif 'channel analytics' in query:
                    url = "https://studio.youtube.com/channel/UCAi-EONczHNaAqr_Ff3HRsA/analytics/tab-overview/period-default"
                    Speak("Channel analytics opening...")
                    webbrowser.get('chrome').open(url)
                    print("Channel analytics opening...")
